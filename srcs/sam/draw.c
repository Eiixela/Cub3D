...
{
	(void)map;
	double player_y_middle = map->player_position->y * SQUARE_SIZE + SQUARE_SIZE / 2;
	double player_x_middle = map->player_position->x * SQUARE_SIZE + SQUARE_SIZE / 2;
	if ((new_y <=(player_y_middle + PLAYER_SIZE/2) && new_y >= (player_y_middle - PLAYER_SIZE/2) && new_x <= (player_x_middle + PLAYER_SIZE/2) && new_x >= (player_x_middle - PLAYER_SIZE/2)))
		return (0);
	return (1);
